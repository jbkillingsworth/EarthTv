...
public class VideoRouter {
    ...
    static final String PROTOBUF_SINK_TOPIC = "video-0";
    ...
    static KafkaStreams buildRouterStream(final String bootstrapServers,
        ...
        streamsConfiguration.put(StreamsConfig.APPLICATION_ID_CONFIG, "video-router-stream-conversion");
        streamsConfiguration.put(StreamsConfig.CLIENT_ID_CONFIG, "video-router-stream-conversion-client");
        streamsConfiguration.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        streamsConfiguration.put(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, schemaRegistryUrl);
        streamsConfiguration.put(KafkaProtobufDeserializerConfig.SPECIFIC_PROTOBUF_VALUE_TYPE, Video.class);
        ...
        final KStream<String, Video> routerStream = builder.stream(JSON_SOURCE_TOPIC);

        // Define the TopicNameExtractor
        TopicNameExtractor<String, Video> orderTopicNameExtractor = (key, video, recordContext) -> {
