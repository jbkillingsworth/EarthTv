...
CREATE OR REPLACE FUNCTION update_frame() RETURNS TRIGGER AS $$
DECLARE
    r RECORD;
	s BOOLEAN;
BEGIN
    FOR r IN SELECT frame_id FROM frame WHERE status = 0 LOOP
		select (
			(select count(image_id) from image where (frame_id = r.frame_id) and (status = 1)) =
			(select count(image_id) from image where image_id = r.image_id)
		) INTO s;
		IF s THEN
			UPDATE frame SET status=1 WHERE frame_id=r.frame_id;
		END IF;
    END LOOP;
	RETURN NULL;
END;
...
CREATE OR REPLACE TRIGGER update_frame_trigger
AFTER UPDATE ON image
FOR EACH ROW
EXECUTE FUNCTION update_frame();

ALTER TABLE frame REPLICA IDENTITY FULL;